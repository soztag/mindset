...
bibliography: 
  - SOZTAG-EXPL2019.bib
  - packages.bib 
nocite: "`r paste0('@', names(bibtex::read.bib(file = 'packages.bib')), collapse = ', ')`"
editor_options: 
...
Die unten stehende Abbildung \@ref(fig:books) zeigt die Verwendung des Begriffs "mindset" in im englischen und deutschen Korpus. 

```{r books, fig.cap="Google Books Unigrams", eval = TRUE}
...
ggplot(
  ...
  labs(linetype = "Korpus", subtitle = "Foo") + labs(colour = "Word") +
...
Spezifischere n-Grams wie etwa "growth mindset" (nach @Dweck2017) kommen leider in den zur Verfügung stehenden Korpora bis 2010 nicht vor.[^grenze]
...
```{r, fig.cap="Google Trends von Mindset Begriffen (LOESSS)"}
...
gtrends2 <- function(geo, subset = "related_queries") {
  ...
  if (subset == "related_queries") {
    trends <- trends %>%
      ...
      dplyr::mutate(subject = as.integer(subject))
  }
  trends
}
