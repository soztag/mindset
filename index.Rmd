...
```{r search_topics, fig.cap="Google Trends von Mindset Begriffen (LOESSS)", fig.height=9}
...
df2graph <- function(df, browser_geo = geos["Welt"]) {
  df <- dplyr::filter(.data = df, browser_geo == browser_geo) %>% 
...
graph2plot <- function(graph, browser_geo = geos["Welt"]) {
  ggraph::ggraph(graph = graph) +
    ...
    ggplot2::ggtitle(
      label = glue::glue("Verwandte Suchanfragen auf Google ({browser_geo})"), 
      subtitle = "Ungerichteter, Partieller Graph"
    )
}

df2plot <- function(df, browser_geo = geos["Welt"]) {
  df2graph(df = df, browser_geo = browser_geo) %>% 
    graph2plot(browser_geo = browser_geo)
}
```

```{r related_terms_world, fig.cap="Google Trends Related Queries Weltweit", eval=TRUE}
readr::read_rds("related_queries.rds") %>% 
  df2plot()
```

```{r related_terms_germany, fig.cap="Google Trends Related Queries (aus Deutschland)"}
readr::read_rds("related_queries.rds") %>% 
  df2plot(browser_geo = geos["Deutschland"])
```
